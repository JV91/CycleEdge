...
async function fetchMSTRHistory() {
    // Load from bundled static file — updated periodically in the repo.
    // Avoids CORS issues with Yahoo Finance when hosted on GitHub Pages.
    const res = await fetch('data/mstr.json');
    if (!res.ok) throw new Error(`MSTR data load error ${res.status}`);
    return await res.json(); // [{x: ms, y: closeUSD}]
}
